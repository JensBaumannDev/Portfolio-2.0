...
@Component({
  selector: 'app-about',
  imports: [TranslatePipe, RouterLink, NgOptimizedImage, NgIconComponent, Reveal],
...
export class About {
  ...
  readonly activeStack = signal<'frontend' | 'backend'>('frontend');

  readonly activeSkills = computed<Skill[]>(() =>
    this.activeStack() === 'frontend' ? this.frontendSkills : this.learningSkills
  );

  selectStack(stack: 'frontend' | 'backend'): void {
    this.activeStack.set(stack);
  }
}
